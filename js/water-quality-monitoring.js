...
fetch('data/floating_solar.json')
    ...
    .then(function(data) {
        floatingSolarNames = data.reservoirs.map(function(r) {
            return r.name;
        });
    })
    .catch(function() {});
...
document.querySelectorAll('.main-tab').forEach(function(btn) {
    btn.addEventListener('click', function() {
        ...
        document.querySelectorAll('.main-content').forEach(function(c) {
...
function rtZoomOverview() {
    if (rtMap) {
        rtMap.closePopup();
        rtMap.setView([25.033964, 121.564468], 8);
    }
}
...
function loadRealtimeData() {
    Promise.all([
        ...
        if (rtMerged.length > 0) {
            var times = rtMerged.map(function(d) {
                return d.time;
            }).sort().reverse();
...
function renderRtMap() {
    Object.values(rtMarkers).forEach(function(m) {
        rtMap.removeLayer(m);
    });
    rtMarkers = {};
...
function loadReservoirs(year) {
    ...
    return loadWaterUsageData('2024')
        .then(function() {
            return fetch('data/' + year + '/list.json');
        })
        .then(function(response) {
            if (!response.ok) {
                ...
                populateYearSelect(currentYear);
                return fetch('data/' + fallback + '/list.json')
                    .then(function(r) {
                        return r.json();
                    });
            }
        ...
        .then(function(list) {
            ...
            var promises = reservoirsList.map(function(reservoir) {
                return fetch('data/' + year + '/' + reservoir + '.json')
                    .then(function(r) {
                        return r.json();
                    })
                    .then(function(data) {
                        ...
                        return fetch('images/' + reservoir + '.svg')
                            .then(function(svgR) {
        ...
        .then(function() {
            allReservoirsData.sort(function(a, b) {
                ...
                var locationKeysA = Object.keys(a.data).filter(function(key) {
                    return key !== 'name' && key !== 'svg';
                });
                ...
                var locationKeysB = Object.keys(b.data).filter(function(key) {
                    return key !== 'name' && key !== 'svg';
                });
...
function renderReservoirsGrid(reservoirs) {
    ...
    reservoirs.forEach(function(reservoir) {
        ...
        var locationKeys = Object.keys(reservoir.data).filter(function(key) {
            return key !== 'name' && key !== 'svg';
        });
        var hasData = false;
        ...
        if (locationKeys.length > 0) {
            ...
            if (locationData.data && Object.keys(locationData.data).length > 0) {
                ...
                var ph = latestData.find(function(item) {
                    return item.itemname === 'pH';
                });
...
function showReservoirDetail(reservoir) {
    ...
    if (locationKeys.length > 0) {
        ...
        if (allDates.length > 1) {
            var uniqueDates = allDates.filter(function(v, i, a) {
...
function fetchReservoirYear(reservoirName, year) {
    ...
    return fetch('data/' + year + '/' + encodeURIComponent(reservoirName) + '.json')
        .then(function(r) {
            if (!r.ok) return null;
            return r.json();
        })
    ...
    var datasets = yearDatasets.map(function(yd, i) {
        ...
        return {
            ...
            data: labels.map(function(date) {
                return valueMap[date] !== undefined ? valueMap[date] : null;
            }),
...
function showItemChart(locationKey, itemName, depth, layer, unit, clickedRow) {
    ...
    chartRow.innerHTML = '<td colspan="4"><div class="chart-container"><span class="chart-close">&times;</span>' +
        ...
        '<button class="multi-year-btn" style="padding:4px 12px;border:1px solid #667eea;background:#fff;color:#667eea;border-radius:4px;cursor:pointer;font-size:13px;">近 5 年趨勢</button>' +
        '</div>' +
    ...
    multiYearBtn.addEventListener('click', function() {
        if (showingMultiYear) {
            ...
            multiYearBtn.textContent = '近 5 年趨勢';
        ...
        var fetches = years.map(function(yr) {
            if (yr === currentYear) return Promise.resolve({
                year: yr,
                data: currentReservoirData.data
            });
        ...
        Promise.all(fetches).then(function(results) {
            ...
            results.forEach(function(r) {
                ...
                if (items.length > 0) yearDatasets.push({
                    year: r.year,
                    data: items
                });
            });

            yearDatasets.sort(function(a, b) {
...
function switchView(view, reservoir, locationKeys) {
    document.querySelectorAll('.view-tab-button').forEach(function(btn) {
...
function activateTabByButton(button) {
    ...
    if (isDateTab) {
        ...
        if (parentContainer) {
            parentContainer.querySelectorAll('.tab-button').forEach(function(btn) {
                btn.classList.remove('active');
            });
            parentContainer.querySelectorAll('.tab-content').forEach(function(content) {
                content.classList.remove('active');
            });
        }
    } else {
        document.querySelectorAll('.tab-button:not(.date-tab)').forEach(function(btn) {
            btn.classList.remove('active');
        });
        document.querySelectorAll('.tab-content[id^="tab-"]').forEach(function(content) {
            content.classList.remove('active');
        });
    }
...
function openReservoirByName(name) {
    var reservoir = allReservoirsData.find(function(r) {
        return r.name === name;
    });
...
function initSupplyTab() {
    ...
    Promise.all([
        fetch('data/supply.json').then(function(r) {
            return r.json();
        }),
        fetch('data/plants.json').then(function(r) {
            return r.json();
        }),
...
function renderSupplyBase() {
    ...
    supplyGeoLayer = L.geoJSON(geojson, {
        ...
        onEachFeature: function(feature, layer) {
            ...
            layer.bindTooltip(props.COUNTYNAME + props.TOWNNAME, {
                sticky: true
            });
        }
    }).addTo(supplyMap);
...
function clearSupplyOverlays() {
    supplyPlantMarkers.forEach(function(m) {
        supplyMap.removeLayer(m);
    });
    supplyPlantMarkers = [];
    supplyLines.forEach(function(l) {
        supplyMap.removeLayer(l);
    });
    supplyLines = [];
...
function renderSupplyForReservoir(name) {
    ...
    if (station && station.lat && station.lng) {
        ...
        supplyReservoirMarker = L.marker(reservoirLatLng, {
                icon: rIcon,
                zIndexOffset: 1000
            })
    ...
    resolvedPlants.forEach(function(plant) {
        ...
        var marker = L.marker(plantLatLng, {
                icon: icon
            })
            .bindPopup(popupHtml)
            .addTo(supplyMap);
...
document.getElementById('supplyReservoirSelect').addEventListener('change', function() {
    ...
    if (!name) {
        ...
        if (supplyMap) supplyMap.setView([25.033964, 121.564468], 8);
        return;
    }
